    ...
    try {
        ...
        if (uploadRes.error) {
            ...
            try {
                ...
                return NextResponse.json({ error: 'Storage upload failed: ' + String(uploadRes?.error?.message || uploadRes?.error || 'unknown') }, { status: 500 })
            }
        }

        if (uploadRes.error) {
            return NextResponse.json({ error: 'Storage upload failed: ' + String(uploadRes.error?.message || uploadRes.error) }, { status: 500 })
        }
